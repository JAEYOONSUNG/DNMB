...
for (genbank_file in gb_files) {
  ...
  # check---------------------
  {
    ...
    for(i in (1:nrow(contig_length))){
      if(!is.na(get(paste0("contig_start_",i)))){
        assign(paste("contig","trim",i,sep="_"), get(paste("contig","trim",i,sep="_")) %>% rename_all(~ stringr::str_replace_all(., " |\\/", "")))
      }
      else{}
    }
  ...
  contig_final <- contig_final %>%
    mutate(contig_number = cumsum(gene_number == 1))

  contig_final <- contig_final %>%
    select(contig, contig_number, everything())

  # export faa
  contig_final %>% dplyr::select(locus_tag, translation) %>% drop_na(translation) %>% mutate("locus_tag"=paste0(">",.$locus_tag)) %>% write.table(.,paste0(qdap::beg2char(genbank_file, "."),".faa"), row.names = FALSE, col.names = FALSE, sep = "\n", quote = FALSE)
