...
dnmb_module_block_columns <- function(genbank_table, prefix) {
  ...
  desired_suffixes <- switch(
    ...
    mRNAcal = c(
      ...
      "tir_access_start",
      "tir_access_end",
      "downstream_access_start",
      "downstream_access_end",
      "standby_start",
      "standby_end",
      "ncs_mfe_dg",
      "ncs_mfe_per_nt",
...
dnmb_module_details_mrnacal <- function(genbank_table, base_cols) {
  ...
  out$score_summary <- dnmb_detail_join(
    ...
    dnmb_detail_key_value("NCS_K", if ("mRNAcal_ncs_lysine_codon_count" %in% names(genbank_table)) genbank_table$mRNAcal_ncs_lysine_codon_count[keep] else NA, digits = 0),
    dnmb_detail_key_value("TIRcoreA", if ("mRNAcal_tir_core_a_fraction" %in% names(genbank_table)) genbank_table$mRNAcal_tir_core_a_fraction[keep] else NA, digits = 3),
